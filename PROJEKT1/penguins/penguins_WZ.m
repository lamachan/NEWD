...
figure;
colors = [0.8500 0.3250 0.0980;  % Red for Adelie Penguin
          0.9290 0.6940 0.1250;  % Yellow for Chinstrap Penguin
          0 0.4470 0.7410];      % Blue for Gentoo Penguin
...
title('Liczba pingwinów na każdej wyspie wg gatunku');
xtickangle(45);
grid on;
...
title('Zależność długości dzioba od długości płetwy wg gatunku');
grid on;

% wykres punktowy zależności masy ciała od długości płetwy
figure;
hold on;
for i = 1:numel(unique_species)
    species_indices = strcmp(species, unique_species{i});
...
ylabel('Masa ciała (g)');
legend(unique_species, 'Location', 'best');
title('Zależność masy ciała od długości płetwy wg gatunku');
grid on;